...
G0 = gsp_2dgrid(32);
n = G0.N;
W0 = G0.W; % weight matrix
G0 = gsp_graph(G0.W, G0.coords+.05*rand(size(G0.coords)));
G0 = gsp_graph(G0.W, G0.coords);
figure; gsp_plot_graph(G0);

% data is asssumed to be smooth vary slowly 
X0 = G0.coords'; % data of each node is 2 dim vector of coordinates
...
params.edge_mask = zero_diag(Z0 < 0.02);
...
W2(W2<1e-4) = 0;
% indeed, the new adjacency matrix has around 4 non zeros per row!
figure; imagesc(W2); title(['Average edges/node: ', num2str(nnz(W2)/G0.N)]);
% update weights and plot
G0 = gsp_update_weights(G0, W2);
figure; gsp_plot_graph(G0);
...
params_NN.k = 2 * k + 1;
clock_flann = tic;
[indx, indy, dist, ~, ~, ~, NN, Z_sorted] = gsp_nn_distanz(X0, X0, params_NN);
