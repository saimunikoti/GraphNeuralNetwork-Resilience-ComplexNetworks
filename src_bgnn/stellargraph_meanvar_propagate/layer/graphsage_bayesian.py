...
from typing import List, Tuple, Callable, AnyStr, Union, Arraytype
from ..mapper import (
...
# addition
from ..mapper.sampled_node_generators_bayesian import GraphSAGENodeGenerator
...
class GraphSAGEAggregator(Layer):
    ...
    def calculate_group_sizes(self, input_shape):
        ...
        # print("#############", len(input_shape), input_shape[0] )
    ...
    def build(self, input_shape):
        ...
        """
        # MODIFYING THE SHAPE OF INPUTS FROM APPLY LAYER
        ...
        for ii, g_shape in enumerate(input_shape):
            # print("input_shape==", len(input_shape), ii, g_shape)
    ...
    def _build_group_weights(self, in_shape, out_size, group_idx=0):
        ...
        """
        # print("=== inshape build weights ==", int(in_shape[-1]) )
    ...
    def call(self, inputs, **kwargs):
        ...
        # groupaggflag = inputs[-2]
        # print("hrGROUP agg flag =====", groupaggflag[0])
        ...
        # if groupaggflag == 'varianceflag':
        #     # MODIFYING FOR XGROUPS FROM VARIANCE
        #     for ii, x in enumerate(inputs):
        #         if self.included_weight_groups[ii]:
        #             x_agg = self.group_aggregate_var(x, nodesperhop,
        #                                                  group_idx=ii)  # CALLS AGGREGATE FUNC WITH xgroup, groupindex
        ...
        for ii, x in enumerate(inputs):
            ...
            # print("===inputs to aggr==", ii, len(inputs), (inputs[0].shape), (inputs[1].shape), (nodesperhop[1].shape) )
            if self.included_weight_groups[ii]:
    ...
    def group_aggregate(self, x_neigh, nodesperhop, group_idx=0):
        """
...
class MeanAggregator(GraphSAGEAggregator):
    ...
    def group_aggregate(self, x_group, nodesprob_perhop, group_idx=0):
        """
...
class GraphSAGE:
    ...
    def __call__(self, xin: List, xin_var: List, nodesperhop: List ):
        ...
        Returns:
            Output tensor
        """
        # print("======= xin before ==== ", type(xin), xin[0].shape)
        # print("======= nodesperhope  ==== ", type(nodesperhop), nodesperhop[1].shape )

        # ==== MODIFICATION: SEPARATION OF XIN AND NODESPERHOP FROM INPUT ====
        # nodesperhop = xin[self.max_hops + 1:2 * (self.max_hops + 1)]
        # xin =  xin[0:self.max_hops+1]

        # print("======= xin after ==== ", type(xin[0]), len(xin), xin[0].shape)

        def apply_layer(x: List, nodesperhop: List, flag: List, num_hops: int):
            """
            ...
            for i in range(self.max_hops - num_hops):
                ...
                # layer_out.append(
                #     self._aggs[num_hops]([Dropout(self.dropout)(x[i]), neigh_in]) # iterate over x[] and neigh_in
                # )
                # print("========= nodes perhop ==", len(nodesperhop), nodesperhop[1].shape)

                # MODIFYING THE SELF AGGS BY PASSING NODESPERHOP WITH XIN AND NEIGH_IN
                # print("======= x[i] ==== ", i, (x[i].shape))
                # print("======= neigh in ==== ",i, neigh_in.shape)
                # tempinput_x = [x[i], neigh_in ]
                # print("======= new input x[i] ==== ", i, len(tempinput_x), tempinput_x[0].shape, tempinput_x[1][1].shape )
                # Apply aggregator to head node and neighbour nodes

                # print("=== type of input ===", type(flag), type(neigh_in), type(x[i]))
                # nodesperhop.extend(flag)
                tempvariable = self._aggs[num_hops]([Dropout(self.dropout)(x[i]), neigh_in, flag, nodesperhop])
                # print("temp var===", (tempvarmean.shape), (tempvarvar.shape))
                layer_out.append(tempvariable) # iterate over x[] and neigh_in
        ...
        for layer in range(0, self.max_hops):
            ...
            h_layer  = apply_layer(h_layer, nodesperhop, meanflag, layer)

        # MODIFY HLAYER VAR FROM GRAPHSAGE LAYERS ITERATIVELTY
        # h_layer_var = xin_var
        # varflag = ['varianceflag']
        # # print("======= h_layer_var ==== ", (h_layer_var))
        # for layer in range(0, self.max_hops):
        #     # tempprint = [ind.shape for ind in h_layer_var]
        #     # print("======= h_layer ==== ", tempprint)
        #     # print("====hlayer===", len(h_layer) )
        #     h_layer_var = apply_layer(h_layer_var, nodesperhop, varflag, layer)
    ...
    def _node_model(self):
        ...
        #     if count==len(self.neighbourhood_sizes):
        #         x_inp.append( Input(shape=(self.max_hops+1)) )
        #     else:
        #         x_inp.append( Input(shape=(self.neighbourhood_sizes[count], self.input_feature_size)) )
        # nodes_inp = []
        # for snodes in range(len(self.neighbourhood_sizes)):
        #     nodes_inp.append( Input(shape=(1,)) )

        # nodes_inp = [ Input(shape=(s,)) for s in self.neighbourhood_sizes ]
        nodes_inp=[]
        for countshape in range(len(self.neighbourhood_sizes)):
            if countshape==0:
                nodes_inp.append(Input(shape=(self.neighbourhood_sizes[countshape],)))
            elif countshape==1:
                nodes_inp.append(Input(shape=(1, self.neighbourhood_sizes[countshape],)))
            else:
                tempshape = int(self.neighbourhood_sizes[countshape]/self.neighbourhood_sizes[countshape-1])
                nodes_inp.append(Input(shape=( self.neighbourhood_sizes[countshape-1], tempshape)))
