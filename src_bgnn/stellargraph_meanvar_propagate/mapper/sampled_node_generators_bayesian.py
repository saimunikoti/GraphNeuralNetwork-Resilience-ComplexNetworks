...
from .sequences_bayesian import NodeSequence
...
class BatchedNodeGenerator(Generator):
    ...
    def flow(self, node_ids, targets=None, shuffle=False, seed=None):
        ...
        return NodeSequence(
            self.sample_features,# batch features from GraphSAGENodeGenerator
            self.batch_size,
            node_ilocs,
            targets,
            shuffle=shuffle,
...
class GraphSAGENodeGenerator(BatchedNodeGenerator):
    ...
    def __init__(
        ...
        self._samplers = SeededPerBatch(
            lambda s: SampledBreadthFirstWalk(G, graph_schema=self.schema, seed=s),seed=seed)
    ...
    def sample_features(self, head_nodes, batch_num):
        ...
        # print("===========", len(node_samples), type(node_samples[0]), len(node_samples[0]))

        # The number of samples for each head node (not including itself)
        """ num_full_samples contains all neighbor nodes as: 1*15 + 15*10=150 + 150*5=915 """
        ...
        def get_levels(loc, lsize, samples_per_hop, walks):
            ...
            # print("==walks at level=", len(walks_at_level))
        ...
        """ nodes_per_hop contains nodes in each hop as [headnodes, nodes in hop1, nodes in hop2, ...] """
        nodes_per_hop = get_levels(0, 1, self.num_samples, node_samples)

        node_type = self.head_node_types[0]
        # print("===========", len(nodes_per_hop[0]), len(nodes_per_hop[1]))

        # Get features for sampled nodes
        """ batch_feats contains nodes feature in each hop as [headnodes feature, feature vector of nodes in hop1, ...] """
        batch_feats = [
        ...
        def get_nodetree(loc, lsize, samples_per_hop, walks):
            ...
            nodes_at_level = np.array([np.array(w[loc:end_loc]) for w in walks])

            # print("==nodes at level=", nodes_at_level.shape, )

            if len(samples_per_hop) < 1:
                return [nodes_at_level]

            return [nodes_at_level] + get_nodetree(
                end_loc, lsize * samples_per_hop[0], samples_per_hop[1:], walks
            )

        nodes_per_level = get_nodetree(0, 1, self.num_samples, node_samples)

        def get_nodesperlevel_tree(nodes_per_level):
            nodes_per_level_old = nodes_per_level.copy()
            for countnode in range(len(nodes_per_level) ) :

                if countnode > 1:
                    tempshape = int((nodes_per_level_old[countnode].shape[1]) / (nodes_per_level_old[countnode-1].shape[1]) )
                    nodes_per_level[countnode] = np.reshape( nodes_per_level[countnode], (nodes_per_level[countnode].shape[0],
                                                        nodes_per_level_old[countnode-1].shape[1], tempshape ) )
            # print("====new nodes per level===", nodes_per_level[0].shape,  nodes_per_level[1].shape,
            #                                     nodes_per_level[2].shape, nodes_per_level[3].shape )
        ...
        def get_neighprob(nodes_per_hop):
            ...
            prob_per_level = [prob_per_level[ind].astype('float32') for ind in range(len(nodes_per_hop))]

            Graph = self.graph.to_networkx()
            # prob for HOP 1
            for chead in range( nodes_per_hop[1].shape[0] ):
                prob_per_level[1][chead,:] = \
                np.array([dict(Graph[nodes_per_hop[0][chead,0]][nodes_per_hop[1][chead, cind]])[0]['weight'] \
                           for cind in range(nodes_per_hop[1].shape[-1] )] )

            prob_per_level[1] = np.reshape(prob_per_level[1], \
                                                 (prob_per_level[1].shape[0], 1, prob_per_level[1].shape[1]))

            # print("==== neigh probabilityy 1====", prob_per_level[1][1,:] )

            # PROB FOR HOP 2
            for chead1 in range( nodes_per_hop[1].shape[0] ):
                for chead2 in range( nodes_per_hop[1].shape[1]):
                    prob_per_level[2][chead1,chead2,:] = \
                    np.array([dict(Graph[nodes_per_hop[1][chead1,chead2]][nodes_per_hop[2][chead1, chead2, cind]])[0]['weight'] \
                           for cind in range(nodes_per_hop[2].shape[-1] )] )

            # print("==== neigh probabilityy 2 ====", prob_per_level[2][1,2,:] )
            # PROB FOR HOP 3
            nodes_per_hop[2] = np.reshape(nodes_per_hop[2], (nodes_per_hop[2].shape[0], nodes_per_hop[2].shape[1]*nodes_per_hop[2].shape[2]))
            for chead1 in range( nodes_per_hop[2].shape[0] ):
                for chead2 in range( nodes_per_hop[2].shape[1]):
                    prob_per_level[3][chead1,chead2,:] = \
                    np.array([dict(Graph[nodes_per_hop[2][chead1, chead2]][nodes_per_hop[3][chead1, chead2, cind]])[0]['weight'] \
                           for cind in range(nodes_per_hop[3].shape[-1] )] )

            # print("==== neigh probabilityy 3 ====", prob_per_level[3][1,2,:] )
            return prob_per_level

        nodesprob_per_hop = get_neighprob(nodes_per_level)
        # print("==== nodes prob per hop ====", len(nodesprob_per_hop), nodesprob_per_hop[1].shape, nodesprob_per_hop[-1].shape )
        # MODIFYING batch_feats FOR INCLUDING NODES PER HOP IN FEATURES LIST
        # for count in range(len(nodes_per_hop)):
        #     batch_feats.append(nodes_per_hop[count])
        # tempnodes_perhop = [np.array(layer_nodes) for layer_nodes in nodes_per_hop]
        ...
        return batch_feats # this passes as an input to keras model
