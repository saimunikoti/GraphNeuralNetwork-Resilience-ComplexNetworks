    ...
    def run(self, nodes, n_size, n=1, seed=None, weighted=False):
        ...
        for node in nodes:  # iterate over root nodes
            for _ in range(n):  # do n bounded breadth first walks from each root node
                ...
                q.append((node, 0))
                # print("=== q start==", len(q),q[0], q[len(q)-1])
                while len(q) > 0:
                    ...
                    # print("-==== walk per depth==", len(walk), cur_node)
                    ...
                    # print("-==== sampled neigh per depth==", len(neighbours), neighbours[0])
                    # add them to the back of the queue
                    q.extend((sampled_node, depth) for sampled_node in neighbours)
                    # print("=== q elem==", (q) )
                    # print("===q", len(q), q[0], q[len(q)-1] )
                # finished i-th walk from node so add it to the list of walks as a list
                walks.append(walk)
        # print("-==== walks==", len(walks), len(walks[0]) )
        return walks # WALKS IS NODESPERSAMPLE (916, 40)
